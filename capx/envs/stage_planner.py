...
class StagePlanner:
    ...
    def observe_turn(
        ...
        status = StageStatus.ABORTED if verdict.status.value == "unsafe" else StageStatus.RUNNING
        is_primary_attempt = role in {
            TurnRole.PRIMARY,
            TurnRole.RETRY,
            TurnRole.RESTAGE_RETRY,
        }
        if is_primary_attempt and verdict.status.value == "success":
    ...
    def replan(
        ...
        replacement = tuple(self._builder.replan(task, state, completed, failed_stage, verdict))
